...
class CreamyBaconBuilder:
    ...
    def bake(self):
        self.progress = PizzaProgress.baking
        print 'baking your cremy bacon for {} seconds.'.format(self.baking_time)
        time.sleep(7)
        self.progress = PizzaProgress.ready
        print 'your cremy bacon is ready'
...
def validata_style(builders):
    try:
        pizza_style = raw_input('What pizza would you like, [m]argarita or [c]remy bacon?')
        builder = builders[pizza_style]()
        validate_input = True
    except KeyError as err:
        print 'only m / c are available'
        return (False, None)
    return (True, builder)
